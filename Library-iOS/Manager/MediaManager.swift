...
class MediaManager {
    ...
    static func getAllSystemMediaFunction(sourceType: PHAssetSourceType = [.typeUserLibrary, .typeCloudShared, .typeiTunesSynced]) -> [PHAsset] {
    ...
    static func getImageDataFunction(asset: PHAsset, resultHandler: @escaping ((_ result: UIImage?, _ imageData: Data?) -> Void)) {
    ...
    ///   - asset: PHAsset
    ///   - resultHandler: Callback
    static func getVideoDataFunction(asset: PHAsset, resultHandler: @escaping ((_ result: URL?, _ videoData: Data?) -> Void)) {
        let imageManager = PHImageManager.default()
